...
Prettier mode is enabled in all buffers where `(lambda nil (when (and
\(not prettier-mode) (or (null prettier-mode-ignore-buffer-function)
\(not (funcall prettier-mode-ignore-buffer-function)))
\(prettier--parsers)) (with-temp-message (unless (eq prettier-pre-warm
'none) Prettier pre-warming...) (prettier-mode))))' would do it.
